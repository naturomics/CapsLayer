...
def squash(vector):
    ...
    scalar_factor = squared_norm / (1 + squared_norm) / tf.sqrt(squared_norm + cfg.epsilon)
...
def routing(vote,
    ...
    if method == 'EMRouting':
        ...
        pose = tf.reshape(pose, shape=[cfg.batch_size, 1, 1, num_outputs] + out_caps_shape)
        activation = tf.reshape(activation_prime, shape=[cfg.batch_size, 1, 1, -1])
        return(pose, activation)
    elif method == 'DynamicRouting':
        B = tf.constant(np.zeros([cfg.batch_size, vote.shape[1].value, num_outputs, 1, 1], dtype=np.float32))
        for r_iter in range(cfg.iter_routing):
            with tf.variable_scope('iter_' + str(r_iter)):
                ...
                if r_iter == cfg.iter_routing - 1:
                    ...
                    pose = squash(s)
                    shape = [cfg.batch_size, vote.shape[1].value, num_outputs] + out_caps_shape
...
def M_step(R, activation, vote, regularizer=None):
    '''
    Args:
        alias H = out_caps_shape[0]*out_caps_shape[1]
    ...
    R_sum_i = tf.reduce_sum(R, axis=1, keepdims=True) + cfg.epsilon
    ...
    # line 4
    stddev = tf.sqrt(tf.reduce_sum(R * tf.square(vote - pose), axis=1, keepdims=True) / R_sum_i + cfg.epsilon)

    # line 5, cost: [batch_size, 1, num_outputs, H]
    H = vote.shape[-1].value
    beta_v = tf.get_variable('beta_v', shape=[cfg.batch_size, 1, pose.shape[2].value, H], regularizer=regularizer)
    cost = (beta_v + tf.log(stddev)) * R_sum_i

    # line 6
    beta_a = tf.get_variable('beta_a', shape=[cfg.batch_size, 1, pose.shape[2], 1], regularizer=regularizer)
    activation = tf.nn.sigmoid(cfg.lambda_val * (beta_a - tf.reduce_sum(cost, axis=3, keepdims=True)))
...
def E_step(pose, stddev, activation, vote):
    ...
    peak_height = 1 / (tf.reduce_prod(tf.sqrt(2 * np.pi * var + cfg.epsilon), axis=-1, keepdims=True) + cfg.epsilon)
